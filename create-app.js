...
const { createPosRouter } = require('./routes/pos-routes');
...
function createApp(dataRepository = repository) {
  ...
  app.use('/api', createPosRouter(dataRepository.pos));

  app.get('/api/mvp-state', async (_req, res, next) => {
    try {
      const state = await dataRepository.obtenerEstadoMvp();
  ...
  app.put('/api/mvp-state', async (req, res, next) => {
    try {
      const state = req.body?.state;
      if (!state || typeof state !== 'object' || Array.isArray(state)) {
        throw apiError(400, 'El estado del sistema debe ser un objeto.');
      }
      const result = await dataRepository.guardarEstadoMvp(state);
